...
def test_timed_release_parse_args_rejects_ignored_base_flags() -> None:
    with pytest.raises(ValueError, match="jax-parallelism"):
        parse_args(["--jax-parallelism", "data"])
    with pytest.raises(ValueError, match="reset-env-each-update"):
        parse_args(["--reset-env-each-update"])
    with pytest.raises(ValueError, match="distance-autocurriculum"):
        parse_args(["--distance-autocurriculum"])
    with pytest.raises(ValueError, match="lethal food"):
        parse_args(
            ["--random-food", "--lethal-food-count", "1", "--lethal-food-sources", "1"]
        )
