...
def test_vision_range_curriculum_trains_and_renders_each_stage(
    ...
    assert result["final_checkpoint"] == second_checkpoint
    assert "--load-model" not in captured_train_args[0]
    assert captured_train_args[1][captured_train_args[1].index("--load-model") + 1] == str(
        first_checkpoint
    )
    assert captured_train_args[0][captured_train_args[0].index("--actor-vision-radius") + 1] == "2"
    assert captured_train_args[1][captured_train_args[1].index("--actor-vision-radius") + 1] == "1"
...
def test_stage_validators_reject_non_increasing_curricula() -> None:
    with pytest.raises(ValueError, match="decreasing"):
        workflows.validate_vision_range_stages([2, 2])
