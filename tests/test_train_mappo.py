...
def test_actor_observation_contains_local_grids_border_mask_and_carrying_flag() -> None:
    ...
    torch.testing.assert_close(actor_obs[0, 0, 6:9], torch.zeros(3))
    torch.testing.assert_close(
        actor_obs[0, 0, 9:12],
        torch.zeros(3),
    )
...
def test_actor_vision_patch_is_three_wide_and_two_deep_in_front() -> None:
    grid = torch.tensor(
        [
            [
                [0.0, 1.0, 2.0, 3.0, 4.0],
    ...
    torch.testing.assert_close(
        patch[0, 0],
        torch.tensor([13.0, 23.0, 33.0, 14.0, 24.0, 34.0]),
    )
...
def test_actor_observation_exposes_one_local_write_bit_patch() -> None:
    ...
    expected_bit = torch.tensor([0, 1, 0.0])
...
def test_local_food_patches_pad_map_edges() -> None:
    ...
    assert patches.shape == (1, 2, 3)
    torch.testing.assert_close(
        patches[0, 0],
        torch.tensor([0, 2 / 3, 0.0]),
    )
    torch.testing.assert_close(
        patches[0, 1],
        torch.zeros(3),
...
def test_checkpoint_can_resume_on_larger_padded_map(tmp_path) -> None:
    ...
    assert checkpoint["actor_obs_dim"] == 31
