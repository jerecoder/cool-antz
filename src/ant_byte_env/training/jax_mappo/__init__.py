"""JAX MAPPO training utilities for AntByte.

The exports are loaded lazily so pure helpers can be imported on machines that
do not have the optional JAX dependency installed.
"""

from __future__ import annotations

from importlib import import_module
from typing import Any

_EXPORTS = {
    "AdamState": ("types", "AdamState"),
    "CRITIC_ARCHITECTURES": ("cli", "CRITIC_ARCHITECTURES"),
    "JaxMAPPOParams": ("types", "JaxMAPPOParams"),
    "LinearParams": ("types", "LinearParams"),
    "Rollout": ("types", "Rollout"),
    "TrainingBatch": ("types", "TrainingBatch"),
    "Transition": ("types", "Transition"),
    "UpdateMetrics": ("types", "UpdateMetrics"),
    "build_actor_observations": ("observations", "build_actor_observations"),
    "build_central_observations": ("observations", "build_central_observations"),
    "build_local_border_patches": ("observations", "build_local_border_patches"),
    "build_local_byte_bit_patches": ("observations", "build_local_byte_bit_patches"),
    "build_local_grid_patches": ("observations", "build_local_grid_patches"),
    "build_local_hub_patches": ("observations", "build_local_hub_patches"),
    "checkpoint_args": ("checkpointing", "checkpoint_args"),
    "collect_rollout": ("rollout", "collect_rollout"),
    "compute_forage_curriculum_rewards": ("rewards", "compute_forage_curriculum_rewards"),
    "compute_gae": ("updates", "compute_gae"),
    "compute_terminal_write_entropy_bonus": ("rewards", "compute_terminal_write_entropy_bonus"),
    "compute_write_bit_entropy_bonus": ("rewards", "compute_write_bit_entropy_bonus"),
    "compute_write_bit_penalties": ("rewards", "compute_write_bit_penalties"),
    "evaluate_actions": ("policy", "evaluate_actions"),
    "evaluate_checkpoint": ("evaluation", "evaluate_checkpoint"),
    "evaluate_params": ("evaluation", "evaluate_params"),
    "flatten_agent_actions": ("observations", "flatten_agent_actions"),
    "food_observation_scale": ("observations", "food_observation_scale"),
    "get_action_and_value": ("policy", "get_action_and_value"),
    "get_action_logits": ("models", "get_action_logits"),
    "get_value": ("models", "get_value"),
    "init_adam_state": ("updates", "init_adam_state"),
    "init_agent_params": ("models", "init_agent_params"),
    "init_layer": ("models", "init_layer"),
    "load_checkpoint": ("checkpointing", "load_checkpoint"),
    "load_checkpoint_for_training": ("transfer", "load_checkpoint_for_training"),
    "main": ("runner", "main"),
    "parse_args": ("cli", "parse_args"),
    "probe_communication_checkpoint": ("probe", "probe_communication_checkpoint"),
    "repeated_write_action_indices": ("transfer_shapes", "repeated_write_action_indices"),
    "reset_batch": ("curriculum", "reset_batch"),
    "save_checkpoint": ("checkpointing", "save_checkpoint"),
    "update_agent": ("updates", "update_agent"),
    "warm_start_actor_params": ("transfer", "warm_start_actor_params"),
}

__all__ = sorted([*_EXPORTS, "write_value_count"])


def __getattr__(name: str) -> Any:
    if name == "write_value_count":
        from ant_byte_env import write_value_count

        return write_value_count
    if name not in _EXPORTS:
        raise AttributeError(name)
    module_name, attr_name = _EXPORTS[name]
    module = import_module(f"ant_byte_env.training.jax_mappo.{module_name}")
    return getattr(module, attr_name)
