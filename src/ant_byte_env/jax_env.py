...
class JaxAntByteForagingEnv:
    ...
    def step(
        ...
        def scan_ant(carry, ant_index):
            ...
            next_pos = self._move_position(ants_pos[ant_index], action=move)
    ...
    def _initial_hub_pos(self, key: jax.Array, hub_pos: jax.Array | None) -> jax.Array:
        ...
        if self.random_hub:
            ...
            return jnp.stack(
                [
                    jax.random.randint(x_key, (), 0, self.width, dtype=jnp.int32),
                    jax.random.randint(y_key, (), 0, self.height, dtype=jnp.int32),
                ]
            )
