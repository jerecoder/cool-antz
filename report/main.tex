...
\subsection{Laberintos: entrenamiento exploratorio sin forrajeo comparable}
...
Por eso el laberinto debe contarse como rama entrenada de exploración, no como resultado comparable de forrajeo cooperativo. La prueba 100x100 debe contarse como fallo de transferencia solo-actor, no como fallo de percepción de paredes. La infraestructura de obstáculos funcionó y produjo evidencia visual, pero el contrato entrenado de laberinto no medía entrega; la decisión experimental fue volver al forrajeo abierto, donde sí existían curvas, videos, puntos guardados y métricas de comida recolectada.
...
\caption{Pruebas de memoria R8--R11. Cada grupo compara la evaluación normal contra dos ablaciones: impedir la lectura de bytes y bloquear la escritura. R12 queda fuera de la figura porque se interrumpió antes de un resultado final confiable. La lectura importante no es cuántos bytes aparecen, sino si la política mantiene forrajeo y mejora frente a las ablaciones.}
...
\begin{itemize}
    ...
    \item el flujo de laberintos como evidencia comparable de forrajeo: hubo video W\&B de exploración 50x50 y una prueba de estrés 100x100, pero ninguna muestra entrega cooperativa entrenada en laberinto;
    ...
    \item las ramas adversariales, timed-release y lethal-cookies como resultados principales: quedan ubicadas en el árbol por refs y commits, pero no tienen el mismo paquete de métricas, videos y evaluaciones que las líneas centrales;
...
\begin{enumerate}
    ...
    \item regenerar la tabla cloud de W\&B después de \run{wandb relogin}, porque esta auditoría solo pudo garantizar exhaustividad local;
    \item inspeccionar manualmente el bucket \run{unclassified-review-needed} antes de usarlo como evidencia en el paper;
    ...
    \item repetir laberintos con contrato de forrajeo comparable si se quiere convertir la prueba de estrés en experimento principal: misma plantilla de métricas, 60 hormigas, fuente lejana, recogidas, entregas, eficiencia y tiempo de convergencia.
\end{enumerate}
